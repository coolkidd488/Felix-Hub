--[[
╔══════════════════════════════════════════════════════════╗
║               FELIX HUB v2.3 - DOORS EDITION             ║
╚══════════════════════════════════════════════════════════╝
  FIXES v2.3:
  • Isqueiro/Vela: filtro rigoroso — só items cujo pai DIRETO é Model de sala
    numerada (não dentro de estantes/gavetas/modelos)
  • Botão F: sem quadrado, posição mais acima, frame totalmente circular
  • Hub: 460x350 — menor, não atravessa a tela
  • Livros: scan GLOBAL em todo workspace a cada vez que busca é ativada
  • Billboard: limpeza imediata no AncestryChanged (texto some ao coletar)
  • LocalPlayer: campo numérico + botão ativar speed/jump
  • Auto-gaveta: abre gavetas e coleta moedas/pilhas em paralelo
  • Aviso Rush/Ambush via DescendantAdded global
]]
...
local Cfg = {
    ...
    ESP_Portas    = true,  ESP_Chaves   = true,
    ESP_Alavancas = true,  ESP_Livros   = true,
    ESP_Figure    = true,  ESP_Screech  = true,
    ESP_Itens     = true,  ESP_Baus     = true,
    ESP_Fusiveis  = true,
...
-- Cache: UID -> true (anti-dup)
-- BBCache: UID -> {gui, conn} (para limpeza garantida)
local Cache   = {}
local BBCache = {}
...
local function TW(o,i,p) if o then TweenService:Create(o,i,p):Play() end end
...
-- ══════════════════════════════════════════════════════
--  FILTRO DE ITEM SOLTO
--  O pai DIRETO do item deve ser um Model cujo nome é
--  número (sala numerada) — assim excluímos itens dentro
--  de estantes, gavetas, outros modelos decorativos.
-- ══════════════════════════════════════════════════════
local function ItemSolto(obj)
    local p = obj.Parent
    if not p then return false end
    -- Pai direto é uma sala numerada
    if tonumber(p.Name) ~= nil then return true end
    -- Pai direto é CurrentRooms (raro mas possível)
    if p.Name == "CurrentRooms" then return true end
    -- Pai direto é workspace
    if p == workspace then return true end
    return false
end

-- ══════════════════════════════════
--  BILLBOARD — tamanho 100x22 fixo
-- ══════════════════════════════════
local function LimparBB(uid)
    ...
    -- fallback por nome
    local g = CoreGui:FindFirstChild(uid)
...
local function CriarBB(obj, txt, cor)
    ...
    local bbg = Criar("BillboardGui",{
        ...
        Size=UDim2.new(0,100,0,22),   -- FIXO para todos
        StudsOffset=Vector3.new(0,3,0),
        Parent=CoreGui,
    })
    local fr = Criar("Frame",{
        BackgroundColor3=Color3.fromRGB(8,8,12), BackgroundTransparency=0.2,
        Size=UDim2.new(1,0,1,0), Parent=bbg,
    })
    ...
    -- Limpeza IMEDIATA quando objeto sai do jogo (item coletado)
...
local function ESP(obj, cor, txt)
    ...
    if Cfg.HL_Ativo then
        ...
        local hl = Criar("Highlight",{
            Name="FHL", Adornee=obj, FillColor=cor,
            FillTransparency=0.55, OutlineColor=Color3.fromRGB(255,255,255),
            OutlineTransparency=0.1,
            DepthMode=Enum.HighlightDepthMode.AlwaysOnTop,
            Enabled=true, Parent=obj,
        })
        TW(hl, TweenInfo.new(1.4,Enum.EasingStyle.Sine,Enum.EasingDirection.InOut,-1,true),
    ...
    if Cfg.BB_Ativo then CriarBB(obj,txt,cor) end
end

-- ══════════════════════════════════
--  NÚMERO VISUAL (+1)
-- ══════════════════════════════════
local function NumVis(nome)
...
-- LIVROS: busca no workspace inteiro quando ativado
...
local function PLivro(o)
    ...
    if Estado.LivrosN >= 8 then
        Estado.ProcurandoLivros = false
        Notif("📚 Felix Hub","Todos os 8 livros!",5)
    end
end
...
-- Entidades de aviso (Rush, Ambush, etc.)
local AvisoNomes = {
    Rush        = "🔴 RUSH vindo!",
    RushMoving  = "🔴 RUSH vindo!",
    Ambush      = "🔴 AMBUSH!",
    AmbushMoving= "🔴 AMBUSH!",
    Halt        = "⚠️ Halt!",
    HaltMoving  = "⚠️ Halt!",
...
local function PEntidade(o)
    if not Cfg.Aviso_Ativo then return end
    local msg = AvisoNomes[o.Name]
    ...
    pcall(function()
        local hl = Criar("Highlight",{
            Name="FHL", Adornee=o, FillColor=Cfg.cPerigo,
    ...
    CriarBB(o, o.Name, Cfg.cPerigo)
    Notif("⚠️ "..o.Name, msg, 5)
end

-- Itens do chão — filtro ItemSolto rigoroso
local NItens = {
...
local function PItem(o)
    ...
    if not NItens[o.Name] then return end
    if not ItemSolto(o) then return end   -- ← filtro: não está dentro de móvel
...
local function PBau(o)
    ...
    if not NBaus[o.Name] then return end
    ESP(o, Cfg.cBau, NBaus[o.Name])
end
...
local function PFusivel(o)
    ...
    if not NFus[o.Name] then return end
    ESP(o, Cfg.cFusivel, "FUSIVEL")
end
...
local function Rotear(o)
    ...
    local n=o.Name
    if AvisoNomes[n]     then PEntidade(o) end
...
local GavetaNomes = {Drawer=true,Dresser=true,Cabinet=true,Nightstand=true,Desk=true}
local ColetaNomes = {Coins=true,Battery=true,Gold=true}

local function TentarColetar(o)
    pcall(function()
        local pp = o:FindFirstChildOfClass("ProximityPrompt")
            or (o:FindFirstDescendant and o:FindFirstDescendantOfClass("ProximityPrompt"))
        if pp then fireproximityprompt(pp); return end
        -- fallback touch
        local char=LP.Character
        if char and char.PrimaryPart then
            local p=GetPart(o)
            if p then char:SetPrimaryPartCFrame(CFrame.new(p.Position)) end
        end
    end)
end
...
local function IniciarAutoGaveta()
    ...
    -- Abre todas as gavetas existentes em paralelo
    task.spawn(function()
        for _,o in ipairs(workspace:GetDescendants()) do
    ...
    -- Monitora novas
    GavetaConn = workspace.DescendantAdded:Connect(function(o)
        if GavetaNomes[o.Name] then
            task.wait(0.4); task.spawn(AbrirGaveta,o)
        end
...
local function ProtEyes(v)
    ...
    local function Ap(char)
        ...
        if v then TDOrig=h.TakeDamage; h.TakeDamage=function()end
            Notif("🛡️ Felix Hub","Proteção ativada!",3)
        else
...
--  GUI
-- ══════════════════════════════════════════════
local gAnt=CoreGui:FindFirstChild("FelixHub_GUI")
if gAnt then gAnt:Destroy() end
...
-- ── Botão flutuante ──
-- Apenas um Frame circular. Nada de fundo extra fora do círculo.
local BH=Criar("Frame",{
    Size=UDim2.new(0,44,0,44),
    Position=UDim2.new(0,12,0.28,0),   -- mais acima
...
Criar("ImageLabel",{Image="rbxassetid://5028857472",ImageColor3=Color3.fromRGB(212,175,55),
    ImageTransparency=0.65,BackgroundTransparency=1,
    Size=UDim2.new(2.6,0,2.6,0),Position=UDim2.new(-0.8,0,-0.8,0),ZIndex=0,Parent=BH})
local BtnF=Criar("TextButton",{Text="F",Font=Enum.Font.GothamBold,
    ...
    Size=UDim2.new(1,0,1,0),Parent=BH})

-- ── Janela principal — 460x350 ──
...
Criar("UIGradient",{Color=ColorSequence.new({
    ...
    ColorSequenceKeypoint.new(1,Color3.fromRGB(7,7,11))}),Rotation=120,Parent=Win})
...
Criar("TextLabel",{Text="DOORS EDITION  v2.3",Font=Enum.Font.Gotham,
...
Criar("UIListLayout",{Padding=UDim.new(0,3),HorizontalAlignment=Enum.HorizontalAlignment.Center,Parent=SB})
...
local IAbas={
    {n="ESP",   i="👁"},  {n="Itens",       i="🎒"},
    {n="Local", i="⚡"},  {n="Visual",      i="🎨"},
    {n="Doors", i="🚪"},  {n="Info",        i="ℹ️"},
}
for _,a in ipairs(IAbas) do
    ...
    Criar("UIListLayout",{Padding=UDim.new(0,5),HorizontalAlignment=Enum.HorizontalAlignment.Center,Parent=pn})
    Criar("UIPadding",{PaddingTop=UDim.new(0,6),PaddingLeft=UDim.new(0,6),PaddingRight=UDim.new(0,6),Parent=pn})
    Abas[a.n]={p=pn,b=bt}
end
...
local function Aba(n)
    for _,a in pairs(Abas) do
        a.p.Visible=false
        TW(a.b,TweenInfo.new(0.12),{BackgroundColor3=Color3.fromRGB(14,14,21),TextColor3=Color3.fromRGB(110,100,70)})
    end
    local a=Abas[n]
    if a then
        a.p.Visible=true
        TW(a.b,TweenInfo.new(0.12),{BackgroundColor3=Color3.fromRGB(28,24,7),TextColor3=Color3.fromRGB(212,175,55)})
        AbaAtiva=n
    end
end
...
local function Sec(p,t)
    ...
    Criar("Frame",{Size=UDim2.new(1,0,0,1),Position=UDim2.new(0,0,0.5,0),
        BackgroundColor3=Color3.fromRGB(212,175,55),BackgroundTransparency=0.65,BorderSizePixel=0,Parent=f})
    Criar("TextLabel",{Text="  "..t.."  ",Font=Enum.Font.GothamBold,
        TextColor3=Color3.fromRGB(212,175,55),TextSize=9,BackgroundColor3=Color3.fromRGB(10,10,15),
...
local function Tog(p,t,ini,cb)
    local f=Criar("Frame",{Size=UDim2.new(1,0,0,30),BackgroundColor3=Color3.fromRGB(15,15,22),
    ...
    Criar("TextLabel",{Text=t,Font=Enum.Font.Gotham,TextColor3=Color3.fromRGB(195,185,145),TextSize=11,
...
local function Sld(p,t,mn,mx,in0,cb)
    local f=Criar("Frame",{Size=UDim2.new(1,0,0,46),BackgroundColor3=Color3.fromRGB(15,15,22),
        BorderSizePixel=0,Parent=p})
    Criar("UICorner",{CornerRadius=UDim.new(0,7),Parent=f})
    Criar("UIStroke",{Color=Color3.fromRGB(38,36,26),Thickness=1,Parent=f})
    ...
    local fi=Criar("Frame",{Size=UDim2.new(p0,0,1,0),BackgroundColor3=Color3.fromRGB(212,175,55),
...
-- Input numérico + botão
local function Inp(p,titulo,pad,btnT,cb)
    local f=Criar("Frame",{Size=UDim2.new(1,0,0,58),BackgroundColor3=Color3.fromRGB(15,15,22),
        BorderSizePixel=0,Parent=p})
    Criar("UICorner",{CornerRadius=UDim.new(0,7),Parent=f})
    Criar("UIStroke",{Color=Color3.fromRGB(38,36,26),Thickness=1,Parent=f})
    Criar("TextLabel",{Text=titulo,Font=Enum.Font.Gotham,TextColor3=Color3.fromRGB(195,185,145),TextSize=11,
        BackgroundTransparency=1,Size=UDim2.new(1,-6,0,17),Position=UDim2.new(0,8,0,3),
        TextXAlignment=Enum.TextXAlignment.Left,Parent=f})
...
Tog(eP,"Billboard (labels)",Cfg.BB_Ativo,     function(v) Cfg.BB_Ativo=v end)
Tog(eP,"Distância",         Cfg.Dist_Ativo,   function(v) Cfg.Dist_Ativo=v end)
Tog(eP,"⚠️ Avisos (Rush/Ambush)",Cfg.Aviso_Ativo,function(v) Cfg.Aviso_Ativo=v end)
Sec(eP,"Selecionar")
...
Tog(iP,"🔑 Abrir gavetas + coletar",Cfg.AutoGaveta,function(v)
    Cfg.AutoGaveta=v; IniciarAutoGaveta()
    if v then Notif("Felix Hub","Auto-gaveta ativado!",3) end
end)
...
Inp(lP,"Velocidade:",16,"▶ Ativar",function(v)
    Cfg.WalkSpeedValor=v; AppSpeed(v)
    Notif("⚡ Speed","WalkSpeed: "..v,3)
end)
local rSpd=Criar("TextButton",{Text="↺ Resetar (16)",Font=Enum.Font.Gotham,
...
Criar("UICorner",{CornerRadius=UDim.new(0,7),Parent=rSpd})
Criar("UIStroke",{Color=Color3.fromRGB(38,36,26),Thickness=1,Parent=rSpd})
rSpd.MouseButton1Click:Connect(function() Cfg.WalkSpeedValor=16; AppSpeed(16) end)

Sec(lP,"Jump Power")
Inp(lP,"Jump Power:",50,"▶ Ativar",function(v)
    Cfg.JumpPowerValor=v; AppJump(v)
    Notif("⚡ Jump","JumpPower: "..v,3)
end)
local rJmp=Criar("TextButton",{Text="↺ Resetar (50)",Font=Enum.Font.Gotham,
    TextColor3=Color3.fromRGB(195,185,145),TextSize=11,
    BackgroundColor3=Color3.fromRGB(15,15,22),BorderSizePixel=0,
    Size=UDim2.new(1,0,0,28),Parent=lP})
Criar("UICorner",{CornerRadius=UDim.new(0,7),Parent=rJmp})
Criar("UIStroke",{Color=Color3.fromRGB(38,36,26),Thickness=1,Parent=rJmp})
rJmp.MouseButton1Click:Connect(function() Cfg.JumpPowerValor=50; AppJump(50) end)
...
Sec(dP,"Notificações")
Tog(dP,"Notif. de Portas",Cfg.NotifPortas,function(v) Cfg.NotifPortas=v end)
Sec(dP,"Sala 50")
...
Criar("TextLabel",{
    ...
    Size=UDim2.new(1,0,0,120),TextWrapped=true,TextYAlignment=Enum.TextYAlignment.Top,Parent=nP})

-- ── Arrastar janela ──
local drg,dOff=false,Vector2.zero
Hdr.InputBegan:Connect(function(i)
    if i.UserInputType==Enum.UserInputType.MouseButton1 or i.UserInputType==Enum.UserInputType.Touch then
        drg=true; dOff=Vector2.new(i.Position.X-Win.AbsolutePosition.X,i.Position.Y-Win.AbsolutePosition.Y)
    end
end)
UserInputService.InputChanged:Connect(function(i)
    if drg and(i.UserInputType==Enum.UserInputType.MouseMovement or i.UserInputType==Enum.UserInputType.Touch) then
        local vp=Camera.ViewportSize
        Win.Position=UDim2.new(0,math.clamp(i.Position.X-dOff.X,0,vp.X-Win.AbsoluteSize.X),
            0,math.clamp(i.Position.Y-dOff.Y,0,vp.Y-Win.AbsoluteSize.Y))
    end
end)
UserInputService.InputEnded:Connect(function(i)
...
UserInputService.InputChanged:Connect(function(i)
    if bdg and(i.UserInputType==Enum.UserInputType.MouseMovement or i.UserInputType==Enum.UserInputType.Touch) then
        local vp=Camera.ViewportSize
        BH.Position=UDim2.new(0,math.clamp(i.Position.X-bOff.X,0,vp.X-44),
            0,math.clamp(i.Position.Y-bOff.Y,0,vp.Y-44))
        bMov=true
    end
end)
UserInputService.InputEnded:Connect(function(i)
...
local function Abrir()
    Open=true;Win.Visible=true;Win.Size=UDim2.new(0,20,0,20)
    TW(Win,TweenInfo.new(0.22,Enum.EasingStyle.Back,Enum.EasingDirection.Out),{Size=UDim2.new(0,460,0,350)})
    if not AbaAtiva then Aba("ESP") end
end
local function Fechar()
    Open=false
    TW(Win,TweenInfo.new(0.15,Enum.EasingStyle.Quad,Enum.EasingDirection.In),{Size=UDim2.new(0,20,0,20)})
    task.delay(0.17,function() if not Open then Win.Visible=false end end)
end
local function Toggle() if Open then Fechar() else Abrir() end end
local function Minimizar()
    Mini=not Mini
    if Mini then
        TW(Win,TweenInfo.new(0.15),{Size=UDim2.new(0,460,0,44)})
        Cont.Visible=false;SB.Visible=false
    else
        TW(Win,TweenInfo.new(0.2,Enum.EasingStyle.Back,Enum.EasingDirection.Out),{Size=UDim2.new(0,460,0,350)})
...
task.spawn(function()
    local gl=BH:FindFirstChildOfClass("ImageLabel")
    while SG and SG.Parent do
        if gl then TW(gl,TweenInfo.new(1.6,Enum.EasingStyle.Sine,Enum.EasingDirection.InOut),{ImageTransparency=0.38}) end
        task.wait(1.6)
        if gl then TW(gl,TweenInfo.new(1.6,Enum.EasingStyle.Sine,Enum.EasingDirection.InOut),{ImageTransparency=0.72}) end
        task.wait(1.6)
    end
end)
...
task.delay(0.6,function()
    Notif("⚡ Felix Hub v2.3","Carregado! Clique no F ou RCtrl.",5)
end)
print("[ FELIX HUB v2.3 ] Carregado — F / RCtrl pra abrir")
