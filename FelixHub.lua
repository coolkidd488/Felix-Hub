--[[
╔══════════════════════════════════════════════════════════╗
║               FELIX HUB v2.5 - DOORS EDITION             ║
╚══════════════════════════════════════════════════════════╝
  FIXES v2.5:
  • Hub: posição fixa na esquerda (não some fora da tela)
  • Botão F: quadrado amarelo removido (ClipsDescendants + transparência)
]]
...
-- Processadores
local function PChave(o) if not Cfg.ESP_Ativo or not Cfg.ESP_Chaves then return end; ESP(o,Cfg.cChave,"CHAVE") end
local function PAlavanca(o) if not Cfg.ESP_Ativo or not Cfg.ESP_Alavancas then return end; ESP(o:FindFirstChild("Main") or o,Cfg.cAlavanca,"ALAVANCA") end
        ...
        if o.Name == "LiveHintBook" then
            ...
            if not Cache[uid] then
                ...
                if Estado.LivrosN >= 8 then
                    ...
                    Notif("📚 Felix Hub","Todos os 8 livros encontrados!",5); break
                end
            end
        end
    end
...
local function PLivro(o)
    ...
    Cache[uid]=true; Estado.LivrosN+=1
    ESP(o,Cfg.cLivro,"LIVRO "..Estado.LivrosN.."/8")
    if Estado.LivrosN >= 8 then Estado.ProcurandoLivros=false; Notif("📚 Felix Hub","Todos os 8 livros!",5) end
end
...
local function PScreech(o)
    ...
    if Cfg.RemoverScreech then pcall(function() o:Destroy() end); Notif("Felix Hub","Screech removido!",3); return end
    ESP(o,Cfg.cScreech,"SCREECH!")
    if Cfg.Aviso_Ativo then Notif("⚠️ SCREECH!","Olhe para ele!",4) end
end

local AvisoNomes={Rush="🔴 RUSH!",RushMoving="🔴 RUSH!",Ambush="🔴 AMBUSH!",
    AmbushMoving="🔴 AMBUSH!",Halt="⚠️ Halt!",HaltMoving="⚠️ Halt!",SeekRig="🔴 Seek!",Eyes="👁 Eyes!"}

local function PEntidade(o)
    if not Cfg.Aviso_Ativo then return end
    local msg=AvisoNomes[o.Name]; if not msg then return end
    local uid=o:GetFullName(); if Cache[uid] then return end; Cache[uid]=true
    pcall(function()
        local hl=Criar("Highlight",{Name="FHL",Adornee=o,FillColor=Cfg.cPerigo,FillTransparency=0.4,
        ...
        o.AncestryChanged:Connect(function()
            if not o:IsDescendantOf(game) then pcall(function() hl:Destroy() end); Cache[uid]=nil end
        end)
    end)
...
local NItens={Flashlight="LANTERNA",Lighter="ISQUEIRO",Vitamins="VITAMINA",
...
local function IniciarESP()
    task.spawn(function()
        for _,o in ipairs(workspace:GetDescendants()) do task.spawn(Rotear,o) end
    end)
    workspace.DescendantAdded:Connect(function(o) task.spawn(Rotear,o) end)
end
...
local function TentarColetar(o)
    pcall(function()
        ...
        if char and char.PrimaryPart then
            local p=GetPart(o); if p then char:SetPrimaryPartCFrame(CFrame.new(p.Position)) end
        end
    end)
end
local function AbrirGaveta(o)
    pcall(function()
        local pp=o:FindFirstChildOfClass("ProximityPrompt"); if pp then fireproximityprompt(pp) end
        task.wait(0.25)
...
local function ProtEyes(v)
    ...
    local function Ap(char)
        ...
        if v then TDOrig=h.TakeDamage; h.TakeDamage=function()end; Notif("🛡️","Proteção ativada!",3)
        else if TDOrig then h.TakeDamage=TDOrig; TDOrig=nil end end
    end
    Ap(LP.Character)
...
local function AppSpeed(v) pcall(function()
    local h=LP.Character and LP.Character:FindFirstChildOfClass("Humanoid"); if h then h.WalkSpeed=v end end) end
...
local function TAntiLag(v) pcall(function()
    settings().Rendering.QualityLevel=v and 1 or Enum.QualityLevel.Automatic end) end

LP.CharacterAdded:Connect(function()
    task.wait(0.4); AppSpeed(Cfg.WalkSpeedValor); AppJump(Cfg.JumpPowerValor)
...
-- ── BOTÃO F ──
-- ClipsDescendants=true no Frame garante que o glow não vaza fora do círculo
local BH=Criar("Frame",{
    Size=UDim2.new(0,44,0,44),
    Position=UDim2.new(0,12,0.28,0),       -- esquerda, um pouco acima do meio
    BackgroundColor3=Color3.fromRGB(12,12,18),
    BackgroundTransparency=0,
    BorderSizePixel=0,
    ClipsDescendants=true,                  -- corta tudo fora do círculo
    Parent=SG,
})
...
-- glow dentro do círculo (não vaza por causa do ClipsDescendants)
Criar("ImageLabel",{Image="rbxassetid://5028857472",
    ImageColor3=Color3.fromRGB(212,175,55),ImageTransparency=0.65,
    BackgroundTransparency=1,
    Size=UDim2.new(1,0,1,0),   -- mesmo tamanho do frame, não extrapola
...
-- ── JANELA ──
-- Posição fixa: começa em x=10 (esquerda), y=60 (abaixo da topbar)
local WIN_W, WIN_H = 460, 350
local Win=Criar("Frame",{Name="Win",
    Size=UDim2.new(0,WIN_W,0,WIN_H),
    Position=UDim2.new(0,10,0,60),         -- FIXO: esquerda, não vai pra direita
...
Criar("TextLabel",{Text="DOORS EDITION  v2.5",Font=Enum.Font.Gotham,
...
Tog(eP,"Buscar Livros (scan global)",false,function(v)
    ...
    if v then
        ...
        task.spawn(ScanLivros); Notif("Felix Hub","Escaneando livros...",3)
    end
end)
...
local function Abrir()
    ...
    TW(Win,TweenInfo.new(0.22,Enum.EasingStyle.Back,Enum.EasingDirection.Out),
        {Size=UDim2.new(0,WIN_W,0,WIN_H)})
    if not AbaAtiva then Aba("ESP") end
end
local function Fechar()
    Open=false
    TW(Win,TweenInfo.new(0.15,Enum.EasingStyle.Quad,Enum.EasingDirection.In),
...
task.delay(0.6,function()
    Notif("⚡ Felix Hub v2.5","Carregado! Clique no F ou RCtrl.",5)
end)
print("[FelixHub] v2.5 carregado com sucesso!")
...
if not ok then
    ...
    pcall(function()
        ...
        local lb=Instance.new("TextLabel"); lb.Text="FelixHub ERRO:\n"..tostring(err)
        lb.TextColor3=Color3.fromRGB(255,80,80); lb.BackgroundColor3=Color3.fromRGB(10,10,15)
        lb.Size=UDim2.new(0,340,0,100); lb.Position=UDim2.new(0.5,-170,0.5,-50)
        lb.Font=Enum.Font.Gotham; lb.TextSize=11; lb.TextWrapped=true; lb.Parent=sg
    end)
end
