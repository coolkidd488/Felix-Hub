--[[
╔══════════════════════════════════════════════════════════╗
║               FELIX HUB v2.4 - DOORS EDITION             ║
╚══════════════════════════════════════════════════════════╝
  FIXES v2.4:
  • CoreGui com fallback automático para PlayerGui
    (resolve "não aparece nada" em executors mobile)
  • pcall geral em toda a GUI para capturar erros silenciosos
  • print de diagnóstico em cada etapa para debug
]]

-- pcall geral: qualquer erro vai aparecer no console do executor
local ok, err = pcall(function()

if _G.FelixHubCarregado then warn("[FelixHub] Já carregado.") return end
_G.FelixHubCarregado = true
print("[FelixHub] Iniciando v2.4...")
...
-- Fallback de parent para GUI
-- Alguns executors mobile bloqueiam CoreGui
local function GetGuiParent()
    ...
    if ok2 then
        ...
        warn("[FelixHub] CoreGui bloqueado, usando PlayerGui")
        return LP:WaitForChild("PlayerGui")
    end
end

local GuiParent = GetGuiParent()
print("[FelixHub] GuiParent:", GuiParent.Name)

-- ══════════════════════════════════
--  CONFIG
-- ══════════════════════════════════
local Cfg = {
    ESP_Ativo    = true,  HL_Ativo   = true,
    BB_Ativo     = true,  Dist_Ativo = true,
    Aviso_Ativo  = true,
...
local function TW(o,i,p)
    if o then pcall(function() TweenService:Create(o,i,p):Play() end) end
end
...
-- Item solto = pai direto é sala numerada, CurrentRooms ou workspace
...
local function CriarBB(obj, txt, cor)
    ...
    local lbl = Criar("TextLabel",{
        ...
        TextColor3=cor, TextStrokeColor3=Color3.new(0,0,0),
...
local function ScanLivros()
    ...
    print("[FelixHub] Scan de livros iniciado...")
    for _,o in ipairs(workspace:GetDescendants()) do
        if o.Name == "LiveHintBook" then
            ...
            if not Cache[uid] then
                ...
                print("[FelixHub] Livro encontrado:",Estado.LivrosN)
...
local AvisoNomes = {
    Rush="🔴 RUSH!", RushMoving="🔴 RUSH!",
    Ambush="🔴 AMBUSH!", AmbushMoving="🔴 AMBUSH!",
    Halt="⚠️ Halt!", HaltMoving="⚠️ Halt!",
    SeekRig="🔴 Seek!", Eyes="👁 Eyes!",
}
...
local function PEntidade(o)
    ...
    pcall(function()
        local hl = Criar("Highlight",{
            Name="FHL",Adornee=o,FillColor=Cfg.cPerigo,FillTransparency=0.4,
            OutlineColor=Color3.fromRGB(255,50,50),OutlineTransparency=0,
            DepthMode=Enum.HighlightDepthMode.AlwaysOnTop,Enabled=true,Parent=o,
        })
    ...
    pcall(CriarBB,o,o.Name,Cfg.cPerigo)
...
local function AbrirGaveta(o)
    pcall(function()
        ...
        for _,f in ipairs(o:GetDescendants()) do
            if ColetaNomes[f.Name] then task.spawn(TentarColetar,f) end
        end
    end)
end
...
local function IniciarAutoGaveta()
    ...
    task.spawn(function()
        for _,o in ipairs(workspace:GetDescendants()) do
            if GavetaNomes[o.Name] then task.spawn(AbrirGaveta,o) end
        end
    end)
    GavetaConn=workspace.DescendantAdded:Connect(function(o)
        if GavetaNomes[o.Name] then task.wait(0.4); task.spawn(AbrirGaveta,o) end
        if ColetaNomes[o.Name] and ItemSolto(o) then task.wait(0.2); task.spawn(TentarColetar,o) end
    end)
end
...
local SG=Criar("ScreenGui",{
    Name="FelixHub_GUI", ResetOnSpawn=false,
    IgnoreGuiInset=true, ZIndexBehavior=Enum.ZIndexBehavior.Sibling,
    Parent=GuiParent,
})
print("[FelixHub] ScreenGui criada em:", GuiParent.Name)
...
Criar("ImageLabel",{Image="rbxassetid://5028857472",
    ImageColor3=Color3.fromRGB(212,175,55),ImageTransparency=0.65,
    BackgroundTransparency=1,Size=UDim2.new(2.6,0,2.6,0),
    Position=UDim2.new(-0.8,0,-0.8,0),ZIndex=0,Parent=BH})
...
Criar("TextLabel",{Text="DOORS EDITION  v2.4",Font=Enum.Font.Gotham,
...
UserInputService.InputChanged:Connect(function(i)
    if drg and(i.UserInputType==Enum.UserInputType.MouseMovement or i.UserInputType==Enum.UserInputType.Touch) then
        ...
        Win.Position=UDim2.new(0,
            math.clamp(i.Position.X-dOff.X,0,vp.X-Win.AbsoluteSize.X),0,
            math.clamp(i.Position.Y-dOff.Y,0,vp.Y-Win.AbsoluteSize.Y))
    end
end)
UserInputService.InputEnded:Connect(function(i)
...
BH.MouseEnter:Connect(function() TW(BH,TweenInfo.new(0.13),{BackgroundColor3=Color3.fromRGB(24,20,6)}) end)
BH.MouseLeave:Connect(function() TW(BH,TweenInfo.new(0.13),{BackgroundColor3=Color3.fromRGB(12,12,18)}) end)
task.spawn(function()
...
task.delay(0.6,function()
    Notif("⚡ Felix Hub v2.4","Carregado! Clique no F ou RCtrl.",5)
end)
print("[FelixHub] Tudo carregado com sucesso! v2.4")

end) -- fim do pcall geral

if not ok then
    warn("[FelixHub] ERRO FATAL:", err)
    -- Tenta mostrar o erro na tela
    pcall(function()
        local LP2 = game:GetService("Players").LocalPlayer
        local sg = Instance.new("ScreenGui")
        sg.ResetOnSpawn = false
        sg.Parent = LP2:WaitForChild("PlayerGui")
        local lbl = Instance.new("TextLabel")
        lbl.Text = "FelixHub ERRO:\n"..tostring(err)
        lbl.TextColor3 = Color3.fromRGB(255,80,80)
        lbl.BackgroundColor3 = Color3.fromRGB(10,10,15)
        lbl.Size = UDim2.new(0,340,0,100)
        lbl.Position = UDim2.new(0.5,-170,0.5,-50)
